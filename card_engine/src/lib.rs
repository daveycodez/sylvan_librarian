...
//
// 2026082705 — cmc went `Option<u8>` -> `Option<f32>` on `OracleCard`, and `creature_power` /
// `creature_toughness` went `Option<i8>` -> `Option<f32>` with it; `NumericIndex`'s key went
// `i16` -> `f32`, and `ArithTupleKey` / `BucketBounds` changed too (`power`/`toughness` ->
// `power_bits`/`toughness_bits`). Un-sets print HALVES in all three columns and the integer casts
// truncated each onto the value it would then wrongly ANSWER. The cmc change happens to move
// `size_of::<AOracleCard>` (288 -> 304), so the header would catch that one by luck — but the
// power/toughness change is absorbed by padding (304 both before and after), `APrinting` is
// untouched at 160, and the INDEX types are not measured by the header at all. THIS IS THE CASE
// THE VERSION FIELD EXISTS FOR: without this bump, get_mmap would accept an archive written by
// the old build and read integer cmc/power bytes as f32 bits.
const ARCHIVE_FORMAT_VERSION: u32 = 2026082705;
