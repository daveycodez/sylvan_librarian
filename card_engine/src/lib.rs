...
struct DivergentPrinting {
    ...
    card_name_id: u32,
    /// That same joined name FOLDED and lowercased — the form `exact_name_matches` compares.
    ///
    /// `collate_name`'s parameter is named `folded_lower` and it means it: it strips
    /// non-alphanumerics and does NOT lowercase, so handing it the display name above yields
    /// `TempleGardenTempleGarden`, which no needle can ever equal. Folding is a BUILD-time
    /// operation here (the engine has no runtime accent fold — see `collate_name`'s own note),
    /// and the row already carries the right string in `card_name_folded`, so it is interned
    /// beside the display name rather than recomputed.
...
/// `layout` is a PRINTING-scalar text field, interned through the general string table rather than
/// `coll_vocab`, so it cannot use `build_hybrid_tag_index`'s `Vec<u16>`-of-vocab-ids shape directly
/// -- this groups by the resolved String instead and hands the result to the same storage decision.
///
/// Upstream introduced this index over the CARD (`OracleCard::card_layout_id`, one bucket entry per
/// card) in the same week this branch moved the field onto the `Printing`. Card ids cannot answer
/// `layout:` here: the 81 `reversible_card` printings are second printings of cards whose other
/// printings are `normal`, so a card-keyed bucket would have to put those 71 cards in BOTH buckets
/// and stop being tight, or in one and be wrong. Printing ids are exact.
///
/// Posts BOTH values a printing answers `layout:` with -- its own and, for a reversible printing,
/// its faces' (`DivergentPrinting::face_layout_id`) -- the same union `ValueTotals::layout` posts
/// and `filter::tri` enumerates. `narrow_rec` marks this index TIGHT, so a value it under-posted
/// would not be a slower answer but a missing row.
///
/// Printing ids land in each bucket in ascending order: `offsets` is contiguous and ascending, and
/// cards are walked in order, so the ids are emitted in increasing order overall -- matching every
/// other index's sorted-postings convention.
fn build_layout_hybrid_index(
    cards: &[OracleCard], printings: &[Printing], offsets: &[u32], strings: &[String],
) -> HybridTagIndex {
    ...
    for cid in 0..offsets.len().saturating_sub(1) {
        ...
        for (i, p) in printings[start..offsets[cid + 1] as usize].iter().enumerate() {
            let pid = (start + i) as u32;
            if p.card_layout_id != NONE_STR {
                by_value.entry(strings[p.card_layout_id as usize].clone()).or_default().push(pid);
            }
            // Never the same string twice: no printing in the corpus has a face layout equal to its
            // own, and the divergent record is keyed by the printing's layout (see
            // `DivergentPrinting::printing_layout_id`), so at most one record can match.
            ...
            {
                by_value.entry(strings[d.face_layout_id as usize].clone()).or_default().push(pid);
            }
        }
    }
    hybrid_from_tag_index(by_value, printings.len())
}
...
struct CardIndexes {
    ...
    // Scalar, not a collection (every PRINTING has exactly one layout, plus its faces' layout on
    // the 81 reversible rows) -- narrow_rec's TextExact::Eq arm for it is a direct tight bucket
    // lookup, unlike the CollectionCmp arm above it shares storage with.
    layout:         HybridTagIndex,  // printing space (scalar, not a collection)
    ...
    // card space: the 71 cards whose printings do not all print the CARD's name, each paired with
    // the folded joined name its divergent printings print instead. Postings for the same reason
    // as the line above — see `planes::build_name_divergent_ids`, which also says why the name
    // rides along with the id rather than being looked up in the string table.
    name_divergent: Vec<(u32, String)>,
...
fn narrow_rec(
    ...
    match filter {
        FilterExpr::ExactName(needle) => {
            ...
            //
            // Candidates found through the index are verified here with the same predicate the
            // walk uses, so that half is exact. `tight_narrow_space` still reports ExactName as
            // tight CARD space, which stays sound because it is only a static pre-reject: the Not
            // arm re-reads the `tight` flag this arm actually returns and declines on a loose one.
            ...
            // before the leaf, which does know the divergent name, is ever asked. Measured on
            // api.scryfall.com 2026-08-17: `!"Temple Garden // Temple Garden"`,
            // `!"Mechtitan // Mechtitan"` and `!"Adrix and Nev, Twincasters // Adrix and Nev,
            // Twincasters"` each answer 1 there, and each answered 0 here.
            //
            // `name_divergent` is the 71 cards that have such a printing, so this is a scan of 71
            // and not of the corpus. The alternative fix — make this arm DECLINE when it finds
            // nothing, and let the scan do the work — sends every no-match `!"..."` to a full
            // corpus scan for one query form's sake, and is pinned against by the tight-set tests
            // below; 71 ids cost less.
            let mut divergent_only = false;
            for entry in indexes.name_divergent.iter() {
                let cid = u32::from(entry.0);
                if crate::filter::exact_name_matches(entry.1.as_str(), needle) && !ids.contains(&cid) {
                    ids.push(cid);
                    divergent_only = true;
                }
            }
            ...
            // hands back all 20, which is what this did with the additions above and no flag
            // (measured 20, against Scryfall's 1). Loose sends the card to the per-printing
            // verify, where the leaf's `divergent_of` gate answers for each printing on its own.
            //
            // Only the divergent additions cost tightness. A needle the trigram index answered by
            // itself is still exact at card level — the overwhelmingly common case, and what keeps
            // `!"Lightning Bolt"` on the cheap path.
            if divergent_only {
                ids.sort_unstable();
                return Narrowed::loose(Candidates::Cards(ids));
            }
        ...
        // the same way `frame_data` does: a `HybridTagIndex` bucket per value, in PRINTING space
        // because that is where the field lives on this branch (upstream built the same index over
        // the card -- see `build_layout_hybrid_index` for why card ids cannot answer this).
        // Tight, unlike the collection fields' own Eq/Gt (which need a downstream length check
        // because containment isn't equality for a multi-valued collection): a scalar field's bucket
        ...
        FilterExpr::TextExact { field: TextField::Layout, op: CmpOp::Eq, value } => {
            ...
            match idx.len_of(value.as_str()) {
                // No printing has this layout value: proves the predicate false everywhere.
                None => Narrowed::tight(Candidates::Printings(Vec::new())),
                Some(k) => {
                    if k > *BITS_PROMOTE
                        && idx.is_dense(value.as_str())
                    {
                        return Narrowed::tight(Candidates::PrintingBits(idx.bits(value.as_str(), n_printings)?));
                    }
                    Narrowed::tight(Candidates::Printings(idx.ids_of(value.as_str())?))
                }
            }
        }
...
//   2026081612 — A REVERSIBLE PRINTING ANSWERS FOR THE NAME IT ACTUALLY PRINTS.
//                2026081611 stored the joined name those 81 printings print, and `!"Temple Garden
//                // Temple Garden"` still returned nothing where api.scryfall.com returns the one
//                printing — measured 2026-08-17, and the same for `!"Mechtitan // Mechtitan"` and
//                `!"Adrix and Nev, Twincasters // Adrix and Nev, Twincasters"`.
//                A LEAF-ONLY FIX IS DEAD CODE. `narrow_rec`'s ExactName arm narrows through a
//                trigram index built from the CARD's collated name and reports the result tight;
//                the needle collates to `templegardentemplegarden`, which shares no trigram window
//                with the card's `templegarden`, so the card is dropped before any leaf runs.
//                Two stored things move. `CardIndexes` gains `name_divergent` — the 71 card ids
//                with such a printing, each with the FOLDED joined name, a few KB — so the arm can
//                union them in and scan 71 rather than the corpus. And `DivergentPrinting` gains
//                `card_name_folded_id`, because `collate_name` does not lowercase and the display
//                name folds to `TempleGardenTempleGarden`, which matches nothing.
...
//                header cannot catch it, and upstream spent the number this branch had already
//                spent above.
//   2026082302 (upstream) — new `CardIndexes` field `layout`, a `HybridTagIndex` over the CARD's
//                layout value. A HEAD-ON COLLISION WITH THIS BRANCH, in both the number and the
//                thing it numbers: upstream took 2026082302 for a card-keyed layout index in the
//                same week this branch took 2026082302 for moving layout OFF the card. The index
//                is kept and rebuilt over printings (`build_layout_hybrid_index` now walks
//                `Printing::card_layout_id` and a reversible printing's `face_layout_id`), and
//                `narrow_rec`'s `TextField::Layout` arm posts PRINTING candidates, because a
//                card-keyed bucket cannot answer a predicate whose value varies within the card.
//   2026082707 — multilingual-store's merge of upstream e24a2c7: the multilingual annex layout
//                plus upstream's `ValueTotals` colour tables and the `layout` index.
//   2026082708 — this branch on top of that: layout on the `Printing`, `OracleCard::divergent`,
//                `CardIndexes::name_divergent`, `PrintingFace::card_watermark_id`, and a `layout`
//                index whose postings are printing ids rather than card ids. Neither parent's
//                number describes the result -- 2026082302 means two different layouts depending
//                on which history wrote it, and 2026082707 has the card-keyed index -- so the
//                merged layout takes a fresh value that leads both.
const ARCHIVE_FORMAT_VERSION: u32 = 2026082708;
...
impl QueryEngine {
            ...
            // rows and nothing else.
            {
                ...
                if same {
                    ...
                    card.divergent.push(DivergentPrinting {
                        ...
                        card_name_id: row.card_name_id,
                        // Interned from the ROW, which already carries the folded form — see the
                        // field's own note on why folding it here rather than at query time is
                        // the only option this engine has.
                        card_name_folded_id: {
                            interner.strings.push(row.card_name_folded.as_str().to_owned());
                            (interner.strings.len() - 1) as u32
                        },
    ...
    let flavor_names_collated: Vec<String> =
        ...
        let indexes = CardIndexes {
            ...
            layout:         build_layout_hybrid_index(&cards, &printings, &offsets, &strings),
            ...
            name_divergent: crate::planes::build_name_divergent_ids(&cards, &strings),
