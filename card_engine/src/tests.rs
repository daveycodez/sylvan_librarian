...
fn fuzz_store_n(rng: &mut rand::rngs::SmallRng, ncards: usize) -> CardData {
    ...
    for i in 0..ncards {
        ...
        for (j, &word) in words.iter().enumerate() {
            ...
            // field's move fixes is invisible. Every `reversible_card` in the bulk is a second
            // printing of a card whose other printings are `normal` (Temple Garden: 1 of 20), so the
    ...
    // layout: same load-bearing property, and the one this list was missing -- every PRINTING above
    ...
    // Printing-space, and the fuzz store is where that gets exercised: `i + j` makes one card's
    // printings disagree about their layout, so a card-keyed index would disagree with the reference
    // on the very first such leaf.
    data.indexes.layout = build_layout_hybrid_index(&data.cards, &data.printings, &data.offsets, &data.strings);
...
///
/// This is Temple Garden reduced to three rows. All 81 `reversible_card` printings in the bulk are
/// SECOND printings of ordinary cards — the 71 oracle ids behind them carry normal printings too —
/// so while `card_layout_id` hung off the `OracleCard` the group had exactly one layout string and
/// it was whichever printing the commit pass reached first. `is:reversible` could not answer 81, or
/// 71, or anything: it answered whatever that accident produced.
///
...
fn a_cards_printings_each_answer_with_their_own_layout() {
    ...
    // Two ordinary printings and one reversible, the bulk's own ratio in miniature.
...
fn the_two_printing_dependence_tables_agree_on_every_text_field() {
    ...
    assert_eq!(fields.len(), 9, "a TextField was added or removed; classify it in BOTH tables");
...
// `stub_printing` (and any fuzz/bench fixture built from it without overwriting `card_layout_id`)
// leaves the field at `NONE_STR`, same as a real printing would only if `layout:` import ever used
// `intern_opt`. `build_layout_hybrid_index` must skip those printings rather than indexing
...
//
// Printing-space since layout moved onto the `Printing`: the postings are pids, and a reversible
// printing posts its FACES' layout as well as its own (`DivergentPrinting::face_layout_id`), which
// is the union `filter::tri` and `ValueTotals::layout` both enumerate. An index that posted only the
// printing's own value would silently drop rows from a leaf `narrow_rec` marks TIGHT.
#[test]
fn layout_hybrid_index_posts_printings_and_skips_the_layoutless() {
    let mut vocab = VocabInterner::new();
    let mut interner = Interner::new();
    let split_id = interner.intern("split".to_string());
    let reversible_id = interner.intern("reversible_card".to_string());
    let normal_id = interner.intern("normal".to_string());

    let mut card = stub_card(1, TYPE_CREATURE, &[], &mut vocab);
    // The reversible printing's faces answer `layout:normal` too -- Temple Garden's shape.
    card.divergent = vec![DivergentPrinting {
        printing_layout_id: reversible_id,
        card_name_id: NONE_STR,
        card_name_folded_id: NONE_STR,
        face_layout_id: normal_id,
        faces: Vec::new(),
    }];
    let cards = vec![card, stub_card(2, TYPE_CREATURE, &[], &mut vocab)];

    let mut printings = vec![
        stub_printing(1, 1, None), // card 0, no layout at all
        stub_printing(2, 2, None), // card 0, split
        stub_printing(3, 3, None), // card 0, reversible (+ its faces' normal)
        stub_printing(4, 4, None), // card 1, no layout
    ];
    printings[1].card_layout_id = split_id;
    printings[2].card_layout_id = reversible_id;
    let offsets = vec![0u32, 3, 4];

    let idx = build_layout_hybrid_index(&cards, &printings, &offsets, &interner.strings);
    assert_eq!(idx.dense.len() + idx.sparse.len(), 3, "the NONE_STR printings contribute no bucket");

    // Through the archived accessors, so the assertion does not depend on which storage tier a
    // value landed in -- at four rows the crossover puts these on the bitmap side.
    let bytes = rkyv::to_bytes::<Error>(&idx).expect("serialize");
    let archived = rkyv::access::<Archived<HybridTagIndex>, Error>(&bytes).expect("access");
    assert_eq!(archived.ids_of("split"), Some(vec![1u32]), "the printing with a layout is indexed, by pid");
    assert_eq!(archived.ids_of("reversible_card"), Some(vec![2u32]), "and so is the reversible one");
    assert_eq!(archived.ids_of("normal"), Some(vec![2u32]), "which also posts its FACES' layout, under its own pid");
    assert_eq!(archived.ids_of("token"), None, "a value no printing carries is absent, not empty");
}
...
fn watermark_is_per_face_and_every_leaf_still_matches_tri() {
    ...
    let re = FilterExpr::TextRegex {
        ...
        regex: crate::filter::compile_search_regex_for_test("simic"),
    };
    ...
    assert_eq!(
        brute(&FilterExpr::TextRegex {
            field: super::TextField::Watermark,
            regex: crate::filter::compile_search_regex_for_test("^izzet$"),
        }),
        vec![0, 2],
